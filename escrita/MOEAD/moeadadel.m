...
nPop=42 ;   % Population Size (Number of Sub-Problems)
...
for it=1:MaxIt
    ...
    popCrossover = []; % inicia a população vazia para o CROSSOVER
%     popMutacao = [];
...
%         p2=pop(j2);
        ...
        y=empty_individual;
        y.Position=Crossoverantigo(p1.Position,p2.Position,crossover_params);
        
        [y.Cost, ~]=CostFunction(y.Position);
        
        popCrossover = [popCrossover
                y];
...
%            popMutacao
    ...
    if numel(pop)>nArchive
        Extra1=numel(pop)-nArchive;
        ToBeDeleted=randsample(numel(pop),Extra1);
        pop(ToBeDeleted)=[];
    end
