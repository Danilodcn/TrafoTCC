...
from tcc.trafo.CONSTANTES import VARIACOES, VARIAVEIS
...
class Populacao(object):
    def __init__(
        ...
        self.number_of_Neighbors = min(
            max(self.number_of_Neighbors, 2), 15
        )  # TODO porque usei 2 e 15?

    def gera_individuos(self):
        return {
            Individuo(variacoes=self.variacoes)
            for i in range(self.numero_populacao)
        }
    ...
    def gerar_grafico(
        ...
        for j, individuos in vetores.items():
            ...
            ax.scatter(
                massas, perdas, label=f"{label} com {len(perdas)}", color=color
            )
    ...
    def mutacao(self, qtd, numero_individuos, taxa, n_frentes):
        ...
        depois_mutacao = map(
            distribui_argumentos,
            it.zip_longest(
                [], para_mutacao, taxa, fillvalue=Individuo.mutacao_uniforme
            ),
        )
    ...
    def selecao(
        self, individuos: list = None, n_selecionados: int = 0, n_frentes=3
    ):
        if individuos == None:
            individuos = self.individuos.copy()
        ...
        while n < n_selecionados:
            ...
            if iteracao > n_frentes or len(individuos) == 0:
                ...
                selecionados.update(
                    rd.choice(individuos + dominantes, n_a_selecionar, False)
                )
                break
            ...
            if n_dominantes + n > n_selecionados:
                ...
                selecionados.update(
                    rd.choice(dominantes, n_a_selecionar, False)
                )
                n = len(selecionados)
    ...
    def _crossover(self, qtd_heuristico, qtd_aritmetico, numero_individuos):
        ...
        depois_crossover_heuristico = list(
            map(
                ...
                it.zip_longest(
                    [],
                    iter_para_heuristico,
                    fillvalue=Individuo.crossover_heuristico,
                ),
            )
        )
        ...
        depois_crossover_aritmetico = list(
            map(
                ...
                it.zip_longest(
                    [],
                    iter_para_aritmetico,
                    fillvalue=Individuo.crossover_aritmetico,
                ),
            )
        )
        ...
        return self.selecao(
            individuos=depois_crossover_aritmetico
            + depois_crossover_heuristico,
            n_selecionados=numero_individuos,
        )
    ...
    def crossover_heuristico(
        self, qtd_heuristico, numero_individuos, n_frentes
    ):
        # Crossover Heuristico
        # para_heuristico = rd.choice(self.individuos, qtd_heuristico)
        ...
        self.individuos = self.selecao(
            n_selecionados=numero_individuos, n_frentes=n_frentes
        )
    ...
    def crossover_aritmetico(
        ...
        para_aritmetico = self.selecao(
            n_selecionados=qtd_aritmetico, n_frentes=n_frentes
        )
        iter_para_aritmetico = it.permutations(para_aritmetico, 2)
