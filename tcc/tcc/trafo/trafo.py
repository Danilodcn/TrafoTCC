...
from tcc.utils import tabelas, utils


class TrafoError(Exception):
    pass
...
class Trafo(object):
    ...
    def __repr__(self):
        txt = ", ".join(
            [(f"{i} = {self.constantes[i]}") for i in self.constantes.keys()]
        )
        return f"Trafo ({txt})"
    ...
    def inicia_as_variaveis(self, nomes: Dict, items: list[Dict]) -> Dict:
        ...
        for nome in nomes.keys():
            try:
                ...
                raise KeyError(
                    f'"Erro ao iniciar as variáveis: "{nome}" nao existe no dicionário "items" \n'
                )
            except Exception as e:
                raise ValueError(
                    f"Erro desconhecido ocorrido na classe Trafo: {e}"
                )
        return retorno
    ...
    def run(
        ...
        return self.calculo_das_perdas_do_trafo(
            variaveis, penalidade=penalidade
        )

    def calculo_das_perdas_do_trafo(
        self, variaveis: list[dict, List], penalidade=False, debug: bool = False
    ):
        ...
        Ku, LD = tabelas.tabela_2_4(
            numero_degraus
        )  # Dimensões do núcleo em função do número de degraus. pg 48 tese
        LD = np.asarray(
            LD, np.float64
        ...
        Pij = tabelas.perda_magnetica_do_nucleo(
            By
        )  # é a perda magnética específica da densidade de fluxo magnético na culatra [W/kg]
        ...
        R1 = (
            0.02857 * Compbt1 / Fc1 * 1e-3
        )  # Resistencia do cobre no lado da baixa tesão
        R2 = (
            0.02857 * CompAT / Fc2AT * 1e-3
        ...
        if penalidade == 1:
            ...
            if PerdasT > Perda_max:
                f1 = (
                    f1
                    + abs(PerdasT - Perda_max) * f_multiplicativo_perdas
                    + Perda_max
                )
                f2 = (
                    f2
                    + abs(Mativa - Mativa_max) * f_multiplicativo_massas
                    + Mativa_max
                )
...
def faz_nada():
    ...
    L = (
        Mo * N1**2 * So / (hb - 0.45 * dc)
    )  # TODO buscar essa referencia. Indutância: Hayt J.r,W.A Buck J.A 2013 - Eletromagnetismo
    Xb = Wa * L / 1000
...
if __name__ == "__main__":
    x = {
        ...
        "z": 235.72047565797871,
        "n": 7,
        "J1": 1.3021543128344217,
        "J2": 1.5635255416644525,
        "Bm": 1.5794831416883455,
        "Ksw": 6.6443181301936916,
        "kt": 0.48786093826602683,
        "Rjan": 3.5623160916564953,
        "rel": 1.1532825588799454,
        "Jbt": 1.3021543128344217,
        "Jat": 1.5635255416644525,
        "Ke": 0.945,
        "k": 0.505,
        "Ku": 0.923,
        "Bs": 1.8,
        ...
        "V1": 0.22,
        "V2": 13.8,
        "Vf1": 0.12701705922171769,
        "Vf2": 13.8,
        "Vmedio2": 12,
        "Q": 150.00000000000003,
        "Qf": 50,
        "Et": 5.9750518209360495,
        "N1": 21.257900856468094,
        "N2": 2309.6033998644211,
        "conexao": "delta-estrela",
        "Ac": 14200.134594495996,
        "Abc": 15023.496285398611,
        "So": 16280.170589916701,
        "dc": 143.97415389616404,
        "S": 150,
        "Nfases": 3,
        "constantes": [150, 60, 3],
        "tipo": "seco",
        "L1": 138.07121358642132,
        "L2": 125.97738465914354,
        "L3": 110.57215019225399,
        "L4": 92.143458493544983,
        "L5": 69.539516331847224,
        "L6": 40.4567372448221,
        "teta1": 0.28734394299515853,
        "teta2": 0.50536051028415729,
        "teta3": 0.69508385432977249,
        "teta4": 0.87629806116834064,
        "teta5": 1.0667186967433886,
        "teta6": 1.2859603924440046,
        "e1": 20.401574503510474,
        "e2": 14.449019266748714,
        "e3": 11.253518280559966,
        "e4": 9.2089516117657411,
        "e5": 7.7202859597560547,
        "e6": 6.0532073218644626,
        "Prof": 138.17311388841082,
        "d": 167.68744259765717,
        "wc": 138.07121358642132,
        "a": 14.808114505617922,
        "Proffem": 108.80976486814993,
        "Kw": 0.15169676096332629,
        "Aw": 169453.11546824532,
        "Acm2": 0.014200134594495996,
        "Awm2": 0.16945311546824532,
        "ww": 218.10144986280284,
        "hw": 776.9463044598748,
        "D": 356.17266344922416,
        "W": 850.41654048486964,
        "Abj": 17326.336239347864,
        "Aj": 16376.767561579978,
        "hy": 125.39585851224778,
        "By": 1.3695543468743003,
        "H": 1027.7380214843704,
        "Vferc": 3.3098226288079463e7,
        "Bfe": 7.6500000000000013e-6,
        "Mc": 253.20143110380795,
        "Pic": 1.1767627,
        "Wic": 297.958,
        "Vferj": 2.7854148028087359e7,
        "Mj": 213.08423241486832,
        "MT": 466.2856635186763,
        "Pij": 0.84900862,
        "Wij": 180.910355,
        "PESOTOTAL": 466.2856635186763,
        "Po": 502.811768,
        "Ip": 1.31953871,
        "atc": 58.1432,
        "ATc": 135522.438,
        ...
        "Iq": 8.97473049,
        "Io": 9.07121658,
        "Fpocalculado": 0.145464361,
        "I1": 393.64791081110843,
        "Fc1": 302.30511616879591,
        "dfc1": 19.619042496561043,
        "Swind1": 6426.3721879193336,
        "hb": 600.76066997010469,
        "tbt1": 11.766764637011006,
        "tbt2": 23.533529274022012,
        "Dextbt": 191.22097187167918,
        "dmbt": 179.45420723466816,
        "Lmbt": 563.77333744843349,
        "Compbt1": 11984.637712998931,
        "VALbt": 1.0869051888207221e7,
        "Mbt3": 29.346440098159498,
        "I2menor": 3.6231884057971011,
        "I2c": 4.166666666666667,
        "Fc2AT": 2.6649175569150207,
        "dfc2AT": 1.8420310575349219,
        "SwindAT": 6154.902649809319,
        "dAt": 88.524942517532665,
        "Dal": 2.7e-6,
    }
